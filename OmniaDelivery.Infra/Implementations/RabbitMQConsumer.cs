...
{
    public class RabbitMQConsumer : IMessageHandler
    {
        ...
        public RabbitMQConsumer(string host, string username, string password, string exchange, string queuename, string routingKey)
        {
            _host = host;
            _username = username;
            _password = password;
        ...
        private Task<bool> HandleEvent(BasicDeliverEventArgs ea)
        {
            ...
            return _callback.HandleMessageAsync(messageType, body, ea.BasicProperties.Headers);
        }
